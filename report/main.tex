...
\textcolor{Blue}
{\textbf{\small{Marian Sorin-Nicu}}} & \includegraphics[scale=0.5]{Images/PolimiLogo}
\end{tabu}
\end{table}}~\\ [7cm]
...
Since manual frame-by-frame annotation is unavailable, a pseudo-ground-truth is obtained by fitting a degree-3 polynomial to the 283 temporally-consistent detections and using the smooth fit as the reference trajectory (a standard practice when a higher-accuracy reference is not available~[9]).
Table~\ref{tab:gteval} reports the per-frame deviation of raw detections from this reference.

\begin{table}[H]
\centering
\caption{Detection accuracy against polynomial pseudo-ground-truth (283 frames, throw 14).}
\label{tab:gteval}
\begin{tabu} to \textwidth { X[0.6,l] X[0.4,r] }
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
Mean error        & 20.5\,px \\
...
The median error of 17\,px ($\approx 0.37$ ball radii) is consistent with a broadcast-quality detector operating without deep learning.
The tail of larger errors (max 253\,px) corresponds to frames where the ball overlaps the bowler or lane boundary; these outliers are suppressed by the polynomial smoother in the final trajectory.
